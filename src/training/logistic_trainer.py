...
from src.constants import DEFAULT_TEST_SIZE, RANDOM_SEED, TARGET_COLUMN
...
def cross_validate_logistic(
    X_train_raw: np.ndarray,   # sem scaling
    y_train: np.ndarray,
    config: LogisticTrainingConfig,
    n_folds: int = 5,       # número de folds para cross-validation.
) -> dict[str, float]:
    """Realiza cross-validation com Logistic Regression e retorna métricas médias e std.
    O cross-validation deve ser realizado no conjunto de treino antes de aplicar scaling, para evitar data leakage.
    Depois de definir os folds, o scaling deve ser aplicado dentro de cada fold (fit no treino do fold, apply no teste do fold). 
    """
    ...
    return {
        "cv_accuracy_mean": float(np.mean([r["accuracy"] for r in cv_results])),
        "cv_accuracy_std": float(np.std([r["accuracy"] for r in cv_results])),
        "cv_precision_mean": float(np.mean([r["precision"] for r in cv_results])),
        "cv_precision_std": float(np.std([r["precision"] for r in cv_results])),
